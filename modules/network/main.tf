...
resource "aws_internet_gateway" "igw" {
  ...
  tags   = merge(var.tags, { Component = "network-igw" })
}

resource "aws_eip" "nat" {
  domain = "vpc"
  tags   = merge(var.tags, { Component = "network-nat-eip" })
}

resource "aws_nat_gateway" "nat" {
  allocation_id = aws_eip.nat.id
  subnet_id     = aws_subnet.public[0].id
  tags          = merge(var.tags, { Component = "network-nat-gw" })
...
resource "aws_route_table" "public" {
  ...
  tags = merge(var.tags, { Component = "network-public-rt" })
}
...
resource "aws_route_table" "private" {
  ...
  tags = merge(var.tags, { Component = "network-private-rt" })
}
